...
public class ReservationManagement implements RoomReservationInterface{
	...
	public boolean reserve(RoomUsage roomUsage) {
		...
		return jdbc.update(sqlReserve, new Object[]{roomUsage.getRoomId(), roomUsage.getByStaffId(), roomUsage.getNote(), roomUsage.getReservedDate(), roomUsage.getAccessBegin(), roomUsage.getAccessUntil()}) == 1;
	}
	...
	public boolean modify(RoomUsage roomUsage) {
		...
		return jdbc.update(sqlModify, new Object[]{roomUsage.getRoomId(), roomUsage.getByStaffId(), roomUsage.getNote(), roomUsage.getReservedDate(), roomUsage.getAccessBegin(), roomUsage.getAccessUntil(), roomUsage.getUsageId()}) == 1;
	}
	...
	private class RoomUsageMapper implements RowMapper<RoomUsage>{
		...
		public RoomUsage mapRow(ResultSet rs, int i) throws SQLException {
			return new RoomUsage(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getDate(6), rs.getTime(7), rs.getTime(8));
		}}
